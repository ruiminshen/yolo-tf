...
def voc(writer, name_index, profile, item, verify=False):
    root = os.path.expanduser(os.path.expandvars(item['root']))
    ...
    for filename in tqdm.tqdm(filenames):
        ...
        if verify:
            assert verify_imageshape(imagepath, imageshape)
...
def coco(writer, name_index, profile, item, verify=False):
    root = os.path.expanduser(os.path.expandvars(item['root']))
    year = str(item['year'])
    name = profile + year
    ...
    for img in tqdm.tqdm(coco.loadImgs(imgIds)):
        ...
        if not os.path.exists(imagepath):
            tf.logging.warn(imagepath + ' not exists')
            continue
        annIds = coco.getAnnIds(imgIds=img['id'], catIds=catIds, iscrowd=None)
        anns = coco.loadAnns(annIds)
        if len(anns) <= 0:
            tf.logging.warn(imagepath + ' has no object')
            continue
